...
export default function MapView({ scenario, solution, selectedUserId, onSelectUser }) {
  ...
  return (
    <div className="h-[680px] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-soft">
      <MapContainer center={[23.756, 90.401]} zoom={12} scrollWheelZoom className="h-full w-full">
        <TileLayer
        ...
        {scenario.areaNodes.map((area) => (
          <Marker key={area.id} position={[area.lat, area.lng]} icon={areaIcon}>
            <Popup><strong>{area.area}</strong><br />Road graph junction</Popup>
          </Marker>
        ))}
          ...
          return (
            <Marker key={station.id} position={[station.lat, station.lng]} icon={stationIcon}>
              <Popup>
                ...
                Capacity: {station.capacity} L<br />
                Used: {used} L<br />
                Status: {assignedStationIds.has(station.id) ? 'Assigned' : 'Available'}<br />
        ...
        {scenario.users.map((user) => (
          <Marker
            ...
            icon={userIcon}
            eventHandlers={{ click: () => onSelectUser(user.id) }}
          >
            <Popup>
              <strong>{user.name}</strong><br />
              Area: {user.area}<br />
              Vehicle: {user.vehicleType}<br />
              Demand: {user.demand} L<br />
              Current fuel: {user.currentFuel} L<br />
              Mileage: {user.kmPerLiter} km/L<br />
              Manual range: {user.manualRange} km<br />
              Fuel-based range: {user.fuelBasedRange} km<br />
              Priority: {user.priority}<br />
              Active Max Distance: {user.maxDistance} km<br />
        ...
        {pathPositions.length > 1 && (
          <>
            <Polyline positions={pathPositions} pathOptions={{ weight: 6, opacity: 0.85 }} />
            {pathPositions.map((position, index) => (
              <CircleMarker key={index} center={position} radius={5} pathOptions={{ weight: 2 }} />
            ))}
          </>
        )}
      </MapContainer>
